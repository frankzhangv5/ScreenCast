...
DeviceManager::DeviceManager() : QObject(nullptr), m_timer(nullptr)
{
    // 调试阶段禁用本地设备信息加载
    loadPersistentData();
    // qDebug() << "Debug mode: Skipping loading of persistent device data";

    // Load all device plugins
    PluginManager::instance().loadAllPlugins();

    // 获取所有支持的设备类型
    QList<DeviceType> supportedTypes = getSupportedDeviceTypes();

    // 从所有代理聚合设备
    for (DeviceType type : supportedTypes)
    {
        qDebug() << "createDeviceProxy: type=" << static_cast<int>(type);
        DeviceProxy* proxy = createDeviceProxy(type);
        if (proxy)
        {
            m_proxies.append(proxy);
        }
    }
    qDebug() << "createDeviceProxy: m_proxies.size()=" << m_proxies.size();
}
...
void DeviceManager::loadPersistentData()
{
    ...
    if (root.contains("cache"))
    {
        ...
        for (const QJsonValue& value : cacheArray)
        {
            ...
            if (deviceObj.contains("rotation"))
            {
                info.rotation = deviceObj["rotation"].toInt();
            }
...
void DeviceManager::savePersistentData()
{
    ...
    for (auto it = m_deviceCache.cache.begin(); it != m_deviceCache.cache.end(); ++it)
    {
        ...
        // Save rotation
        deviceObj["rotation"] = info.rotation;
...
QVector<DeviceInfo> DeviceManager::getDevicesInternal() const
{
    ...
    for (const auto& dev : m_devices)
    {
        ...
        // Use a hybrid approach: start with cached info if available, but preserve current connection status
        DeviceInfo deviceInfo = cached ? *cached : dev;

        // Always preserve the name from the original device if it's not empty
        // This ensures we don't lose the device name if cache doesn't have it
        if (!dev.name.isEmpty())
        {
            deviceInfo.name = dev.name;
            qDebug() << "Preserving device name from current device:" << dev.name << "for serial:" << dev.serial;
        }
        ...
        // Debug output to verify the final device info
        qDebug() << "Final device info for" << dev.serial << ": name=" << deviceInfo.name
...
QList<DeviceType> DeviceManager::getSupportedDeviceTypes() const
{
    return PluginManager::instance().getSupportedDeviceTypes();
}

QIcon DeviceManager::getDeviceTypeIcon(DeviceType type) const
{
    IDevicePlugin* plugin = PluginManager::instance().getPlugin(type);
    if (plugin)
    {
...
bool DeviceManager::isDeviceTypeSupported(DeviceType type) const
{
    return PluginManager::instance().hasPluginForType(type);
}
...
DeviceProxy* DeviceManager::createDeviceProxy(DeviceType type) const
{
    QMutexLocker locker(&m_mutex);
    ...
    if (plugin)
    {
        return plugin->createDeviceProxy(const_cast<DeviceManager*>(this));
    }
    return nullptr;
}
...
{
#if DEVICE_SCAN_DEBUG
    qDebug() << "DeviceManager::startMonitor";
#endif
    if (!m_timer)
    {
...
{
#if DEVICE_SCAN_DEBUG
    qDebug() << "DeviceManager::stopMonitor";
#endif
    if (m_timer)
    {
        m_timer->stop();
...
{
#if DEVICE_SCAN_DEBUG
    qDebug() << "DeviceManager::refreshDevices";
#endif
    ...
    {
#if DEVICE_SCAN_DEBUG
        qDebug() << "DeviceManager::refreshDevices: proxy=" << static_cast<int>(proxy->deviceType());
#endif
        IDevicePlugin* plugin = PluginManager::instance().getPlugin(proxy->deviceType());
        if (plugin && Settings::instance().isPluginEnabled(plugin->pluginName()))
        {
            newDevices += proxy->listDevices();
#if DEVICE_SCAN_DEBUG
            qDebug() << "DeviceManager::refreshDevices: proxy->listDevices() count=" << newDevices.size();
#endif
        }
    }
    ...
    bool devicesChanged = false;
    {
        ...
        for (const DeviceInfo& dev : newDevices)
        {
            if (!m_devices.contains(dev))
            {
                ...
                emit deviceConnected(dev.serial);
                devicesChanged = true;
            }
        }
        for (const DeviceInfo& dev : m_devices)
        {
            if (!newDevices.contains(dev))
            {
                ...
                emit deviceDisconnected(dev.serial);
                devicesChanged = true;
            }
        }
        if (devicesChanged)
...
void DeviceManager::handleDeviceSetupError(const QString& serial, const QString& errorMessage)
{
    qDebug() << "DeviceManager::handleDeviceSetupError: " << errorMessage << serial;
    emit deviceSetupError(serial, errorMessage);
}

void DeviceManager::processNewDevice(const DeviceInfo& dev)
{
    qDebug() << "DeviceManager::processNewDevice";
    ...
    if (!proxy)
    {
        handleDeviceSetupError(dev.serial, tr("Failed to query proxy information"));
        return;
    }
    ...
    if (!cached)
    {
        // Handle new device
        DeviceInfo info;
        if (!proxy->queryDeviceInfo(dev.serial, info))
        {
            handleDeviceSetupError(dev.serial, tr("Failed to query device information"));
            return;
        }

        if (!proxy->setupMirrorServer(dev.serial, forwardPort))
        {
            handleDeviceSetupError(dev.serial, tr("Failed to setup mirror server"));
            return;
        }
    ...
    else
    {
        ...
        {
            handleDeviceSetupError(dev.serial, "Failed to setup mirror server");
            return;
        }
    }

    // Save persistent data after successful setup
    savePersistentData();
}
