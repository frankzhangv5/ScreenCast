...
def build_qmake_project(target_name, project_file, build_dir, dist_dir, build_type):
    ...
    try:
        # 配置项目
        logi(
            f"Configuring project {project_file} for {target_name} with {build_type} configuration..."
        )
        ...
        # 构建项目
        logi(
            f"Compiling project {project_file} for {target_name} with {build_type} configuration..."
        )
        ...
        result = subprocess.run(
            build_cmd, capture_output=True, text=True, encoding="utf-8", errors="ignore"
        )
        if result.returncode != 0:
...
def deploy_application(
    app_dist_dir, app_target_name, qt_dir, plugin_dist_dir, script_dir
):
    """部署应用程序"""
    ...
    # 根据平台选择插件文件扩展名
    plugin_extensions = (
        [".dll"] if IS_WINDOWS else [".so"] if IS_LINUX else [".dylib", ".so"]
    )
...
def deploy_qt_windows(exe_path, qt_dir):
    ...
    result = subprocess.run(
        deploy_cmd, capture_output=True, text=True, encoding="utf-8", errors="ignore"
    )
    if result.returncode != 0:
        ...
        if lib_dir.exists():
            ...
            for file in lib_dir.iterdir():
                if file.is_file() and any(
                    file.name.endswith(ext) for ext in lib_extensions
                ):
                    try:
    ...
    try:
        if IS_WINDOWS:
            return package_windows(
                app_dist_dir, app_target_name, script_dir, build_type
            )
        elif IS_LINUX:
...
def package_windows(app_dist_dir, app_target_name, script_dir, build_type):
    ...
    iscc_paths = [
        ...
        os.path.join(
            os.environ.get("ProgramFiles", "C:\Program Files"), "Inno Setup 6\ISCC.exe"
        ),
    ]
    iscc_path = None
    ...
    if not iscc_path:
        loge(
            "Inno Setup 6 not found. Please install Inno Setup to create Windows installer."
        )
    ...
    portable_zip_name = (
        f"ScreenCast-v{version}-windows-{arch}.zip"  # 使用动态版本号和架构
    )
    ...
    if portable_source_dir.exists():
        try:
            ...
            if portable_zip_path.exists():
                ...
                logi(
                    f"File size: {os.path.getsize(portable_zip_path) / 1024 / 1024:.2f} MB"
                )
            else:
...
""".format(
    ...
    if result.returncode == 0:
        installer_path = (
            installer_dir / f"{app_name}-v{version}-windows-{arch}-Setup.exe"
        )
...
def package_linux(app_dist_dir, app_target_name, script_dir, build_type):
    ...
    if not (linuxdeploy_tool.exists() and appimagetool.exists()):
        loge(
            "Linux packaging tools not found. Please ensure linuxdeployqt and appimagetool are available in pack/tools/"
        )
...
def package_macos(app_dist_dir, app_target_name, script_dir, build_type):
    """macOS平台打包 - 创建DMG安装镜像"""
    logi("Creating macOS package (DMG)...")

    # 检查是否在macOS环境中运行
    if not IS_MACOS:
        logi(
            "Skipping macOS package creation in non-macOS environment. Please run on macOS for proper packaging."
        )
    ...
    try:
        ...
        arch = platform.machine()
        arch_name = (
            "x86_64" if arch == "x86_64" else ("arm64" if arch == "arm64" else arch)
        )
        ...
        if not app_path.exists():
            loge(f"Application bundle not found: {app_path}")
            return False

        # 使用macdeployqt确保所有依赖都已包含
        qt_dir = find_qt_dir()
        if qt_dir:
            macdeployqt = Path(qt_dir) / "bin/macdeployqt6"
            if macdeployqt.exists():
                logi("Running macdeployqt6 to ensure all dependencies are included...")
                result = subprocess.run(
                    [str(macdeployqt), str(app_path), "-dmg"],
                    capture_output=True,
                    text=True,
                    encoding="utf-8",
                    errors="ignore",
                ...
                if result.returncode != 0:
                    ...
                    logi("macdeployqt completed successfully")
            else:
                loge(f"macdeployqt not found at: {macdeployqt}")
                return False
        else:
        ...
        try:
            # 复制应用程序到临时目录
            logi(f"Copying application to temporary directory...")
            shutil.copytree(app_path, temp_dmg_dir / app_name)

            # 创建Applications链接（macOS安装体验的标准做法）
            logi("Creating Applications symlink...")
            applications_link = temp_dmg_dir / "Applications"
            subprocess.run(
                ["ln", "-s", "/Applications", str(applications_link)], check=True
            )
            ...
            # 创建DMG镜像
            logi(f"Creating DMG image: {dmg_path}")
            # 先创建一个临时的只读DMG
            temp_dmg = str(Path(app_dist_dir).parent / "temp.dmg")
            subprocess.run(
                [
                    "hdiutil",
                    "create",
            ...
            logi(f"DMG package created successfully: {dmg_path}")
            return True
        finally:
            # 确保清理临时目录
            if temp_dmg_dir.exists():
        ...
        loge(f"Error creating macOS package: {e}")
        import traceback
...
def main():
    ...
    parser.add_argument(
        "--qt-dir", default=os.environ.get("QTDIR"), help="Qt安装目录路径"
    )
    parser.add_argument("--clean", action="store_true", help="清理构建目录")
    parser.add_argument("--no-deploy", action="store_true", help="不进行部署")
    parser.add_argument("--package", action="store_true", help="打包生成最终安装包")
    ...
    if IS_WINDOWS:
        if (
            not compiler_path
            or not (Path(compiler_path) / "bin/mingw32-make.exe").exists()
        ):
    ...
    # 构建主应用程序
    app_dist_dir, app_target_name = build_application(
        script_dir, build_dir, dist_dir, build_type
    )
